...
- `contracts/boundary.schema.json`、`contracts/ndjson.schema.json`、Python 生成模型/协议映射、
  TypeScript IPC/事件绑定和 Rust manifest/one-shot/事件适配器都是受 Git 跟踪的生成物；
  运行 `python scripts/generate_contracts.py --check` 可验证逐字节 freshness。
...
当前持久化版本为环境缓存 `15`、工作台预设 `2`、分段清单 `3`。损坏或版本不匹配的数据会被
改名隔离并重建，不会迁移、解析或回退读取；预设加载失败会通过全局 `IssueBanner` 告知用户。
Rust 缓存/预设端口按规范化路径串行化完整事务，同 key 并发读取使用 single-flight，共享一次
隔离、探测/重建和原子提交结果。
