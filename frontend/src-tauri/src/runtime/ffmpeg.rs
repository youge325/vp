...
//!
//! Release 构建中如果 1/2 都没命中,``mod.rs`` 会把 None 视为致命错误并返回
...
pub(super) fn resolve_ffmpeg_tools(
    ...
    let ffmpeg = resolve_tool_path(env_path("VP_FFMPEG_PATH"), runtime_root, "ffmpeg");
    let ffprobe = resolve_tool_path(env_path("VP_FFPROBE_PATH"), runtime_root, "ffprobe");
    (ffmpeg, ffprobe)
}
...
mod tests {
    ...
    #[test]
    fn tool_resolution_preserves_config_then_runtime_priority() {
        ...
        assert_eq!(
            resolve_tool_path(Some(configured.clone()), Some(&runtime_root), "ffmpeg",),
            Some(configured.clone())
        );

        std::fs::remove_file(&configured).expect("remove configured tool");
        assert_eq!(
            resolve_tool_path(Some(configured), Some(&runtime_root), "ffmpeg",),
            Some(runtime.clone())
        );

        std::fs::remove_file(&runtime).expect("remove runtime tool");
        assert_eq!(resolve_tool_path(None, Some(&runtime_root), "ffmpeg"), None);
    }

    #[test]
    fn resolves_ffprobe_from_the_canonical_runtime_layout() {
        let temp = tempfile::tempdir().expect("temp dir");
        let runtime_root = temp.path().join("runtime");
        let bundled = create_tool(&runtime_root, "ffmpeg", "ffprobe");

        assert_eq!(
            resolve_tool_path(None, Some(&runtime_root), "ffprobe"),
            Some(bundled)
        );
    }
}
