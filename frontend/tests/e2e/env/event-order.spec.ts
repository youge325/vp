...
async function setupEventListener(tauriPage: any, eventName: string) {
  await tauriPage.evaluate(async (name: string) => {
    ...
    const handlerId = internals.transformCallback((eventData: any) => {
      // @ts-expect-error
      window.__E2E_EVENTS.push({ name, data: eventData.payload, ts: Date.now() })
    })
...
test.describe('Task event ordering', () => {
  ...
  test('task-progress events precede task-completed', async ({ tauriPage }) => {
    ...
    const progressIndices = events
      .map((e: any, i: number) => (e.name === 'task-progress' ? i : -1))
      .filter((i: number) => i >= 0)
    const completedIndices = events
      .map((e: any, i: number) => (e.name === 'task-completed' ? i : -1))
      .filter((i: number) => i >= 0)
