...
test.describe('Environment check', () => {
  test('returns the structured probe and then serves the typed cache', async ({ tauriPage }) => {
    const first = await invokeTauri<any>(tauriPage, 'check_environment', { forceRefresh: false })
    ...
    const cached = await invokeTauri<any>(tauriPage, 'check_environment', { forceRefresh: false })
