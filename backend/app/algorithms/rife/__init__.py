...
注意: ``RIFESolver`` 实际定义在 ``app.algorithms.rife.solver`` 子模块,
该子模块顶层 ``import torch``。这里通过模块级 ``__getattr__`` 提供
延迟导入,确保 ``import app.algorithms.rife`` 不会无意间把 PyTorch
...
if TYPE_CHECKING:
    from app.algorithms.rife.solver import RIFESolver  # noqa: F401


def __getattr__(name: str):
    if name == "RIFESolver":
        from app.algorithms.rife.solver import RIFESolver as _RIFESolver

        return _RIFESolver
    raise AttributeError(f"module 'app.algorithms.rife' has no attribute {name!r}")
