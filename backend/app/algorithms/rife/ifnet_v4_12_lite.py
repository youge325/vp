"""RIFE v4.12.lite 模型定义。

特点：LeakyReLU 激活、ResConv 残差卷积、nn.Sequential Head(3→32→4)、
4 级 IFBlock、更小的通道数（128/96/64/48）、PixelShuffle 上采样、支持 ensemble。

与 v4.12 的差异：encode 输出 4 通道（非 8），block 通道更小，为轻量版。

参考 vs-rife 的 IFNet_HDv3_v4_12_lite.py 实现。
"""
...
class IFNet(nn.Module):
    ...
    轻量版：encode 输出 4 通道，block 通道 128/96/64/48。
    block0 输入 7+8=15 通道。
    """
