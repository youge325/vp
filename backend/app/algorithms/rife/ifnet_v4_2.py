...
v4.2 相比 v4.1 的变化：PReLU→LeakyReLU，convblock 后不再加 residual。

参考 vs-rife 的 IFNet_HDv3_v4_2.py 实现。
"""
...
class IFBlock(nn.Module):
    ...
    conv 无 residual-add + ConvTranspose2d 上采样，输出 (flow, mask) 2 元组。
    """
...
class IFNet(nn.Module):
    ...
    4 级 IFBlock 级联细化光流，无 Head 编码器，支持 ensemble。
    scale_list 作为实例属性存储。
    """
