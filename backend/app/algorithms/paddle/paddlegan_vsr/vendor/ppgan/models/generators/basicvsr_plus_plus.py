...
from .basicvsr import PixelShufflePack, flow_warp, SPyNet, ResidualBlocksWithInputConv, SecondOrderDeformableAlignment
...
class BasicVSRPlusPlus(nn.Layer):
    ...
    def __init__(self, mid_channels=64, num_blocks=7, is_low_res_input=True):
        ...
        # optical flow
        self.spynet = SPyNet()
        weight_path = get_path_from_url("https://paddlegan.bj.bcebos.com/models/spynet.pdparams")
        ...
        self.backbone_backward_1 = ResidualBlocksWithInputConv(2 * mid_channels, mid_channels, num_blocks)
        self.backbone_forward_1 = ResidualBlocksWithInputConv(3 * mid_channels, mid_channels, num_blocks)
        self.backbone_backward_2 = ResidualBlocksWithInputConv(4 * mid_channels, mid_channels, num_blocks)
        self.backbone_forward_2 = ResidualBlocksWithInputConv(5 * mid_channels, mid_channels, num_blocks)
    ...
    def forward(self, lqs):
        ...
        if self.is_low_res_input:
            ...
            lqs_downsample = F.interpolate(lqs.reshape([-1, c, h, w]), scale_factor=0.25, mode="bicubic").reshape(
                [n, t, c, h // 4, w // 4]
            )
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i > 0:
                ...
                if i > 1:  # second-order features
                    ...
                    flow_n2 = flow_n1 + flow_warp(flow_n2, flow_n1.transpose([0, 2, 3, 1]))

                    cond_n2 = flow_warp(feat_n2, flow_n2.transpose([0, 2, 3, 1]))

                # flow-guided deformable convolution
                cond = paddle.concat([cond_n1, feat_current, cond_n2], axis=1)
                feat_prop = paddle.concat([feat_prop, feat_n2], axis=1)

                feat_prop = self.deform_align_backward_1(feat_prop, cond, flow_n1, flow_n2)

            # concatenate and residual blocks
            feat = [feat_current] + [feats[k][idx] for k in feats if k not in ["spatial", "backward_1"]] + [feat_prop]
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i > 0:
                ...
                feat_prop = self.deform_align_forward_1(feat_prop, cond, flow_n1, flow_n2)

            # concatenate and residual blocks
            feat = [feat_current] + [feats[k][idx] for k in feats if k not in ["spatial", "forward_1"]] + [feat_prop]
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i > 0:
                ...
                feat_prop = self.deform_align_backward_2(feat_prop, cond, flow_n1, flow_n2)

            # concatenate and residual blocks
            feat = [feat_current] + [feats[k][idx] for k in feats if k not in ["spatial", "backward_2"]] + [feat_prop]
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i > 0:
                ...
                feat_prop = self.deform_align_forward_2(feat_prop, cond, flow_n1, flow_n2)

            # concatenate and residual blocks
            feat = [feat_current] + [feats[k][idx] for k in feats if k not in ["spatial", "forward_2"]] + [feat_prop]
