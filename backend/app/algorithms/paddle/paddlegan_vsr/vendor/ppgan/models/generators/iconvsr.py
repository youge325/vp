...
from .basicvsr import SPyNet, PixelShufflePack, ResidualBlockNoBN, ResidualBlocksWithInputConv, flow_warp
...
class IconVSR(nn.Layer):
    ...
    def __init__(self, mid_channels=64, num_blocks=30, padding=2, keyframe_stride=5):
        ...
        edvr_wight_path = get_path_from_url("https://paddlegan.bj.bcebos.com/models/edvrm.pdparams")
        self.edvr.set_state_dict(paddle.load(edvr_wight_path))

        self.backward_fusion = nn.Conv2D(2 * mid_channels, mid_channels, 3, 1, 1, bias_attr=True)
        self.forward_fusion = nn.Conv2D(2 * mid_channels, mid_channels, 3, 1, 1, bias_attr=True)
    ...
    def compute_refill_features(self, lrs, keyframe_idx):
        ...
        if self.padding == 2:
            lrs = [lrs[:, 4:5, :, :], lrs[:, 3:4, :, :], lrs, lrs[:, -4:-3, :, :], lrs[:, -5:-4, :, :]]
        elif self.padding == 3:
    ...
    def forward(self, lrs):
        ...
        assert h_input >= 64 and w_input >= 64, (
            f"The height and width of inputs should be at least 64, but got {h_input} and {w_input}."
        )
        ...
        feat_prop = paddle.to_tensor(np.zeros([n, self.mid_channels, h, w], "float32"))
...
class EDVRFeatureExtractor(nn.Layer):
    ...
    def __init__(
        ...
        self.feature_extraction = make_layer(ResidualBlockNoBN, num_blocks_extraction, nf=mid_channels)
