...
from .basicvsr import PixelShufflePack, flow_warp, ResidualBlocksWithInputConv
from ...utils.download import get_path_from_url
from ...modules.init import constant_
...
class MSVSR(nn.Layer):
    ...
    def __init__(
        ...
        if use_tiny_spynet:
            ...
            weight_path = get_path_from_url("https://paddlegan.bj.bcebos.com/models/modified_spynet_tiny.pdparams")
            self.spynet.set_state_dict(paddle.load(weight_path))
        else:
            self.spynet = ModifiedSPyNet(num_blocks=6, use_tiny_block=False)
            weight_path = get_path_from_url("https://paddlegan.bj.bcebos.com/models/modified_spynet.pdparams")
            self.spynet.set_state_dict(paddle.load(weight_path))

        # feature extraction module
        self.feat_extract = ResidualBlocksWithInputConv(3, mid_channels, num_init_blocks)

        # propagation branches module for stage2 and stage3
        self.deform_align = nn.LayerDict()
        self.backbone = nn.LayerDict()

        prop_names = ["stage2_backward", "stage2_forward", "stage3_backward", "stage3_forward"]

        for i, layer in enumerate(prop_names):
            if i > 1 and self.use_refine_align:
                self.deform_align[layer] = ReAlignmentModule(
                    mid_channels, mid_channels, 3, padding=1, deformable_groups=deform_groups
                )
            else:
            ...
            self.backbone[layer] = ResidualBlocksWithInputConv((3 + i) * mid_channels, mid_channels, num_blocks)

        # stage1
        self.stage1_align = AlignmentModule(mid_channels, mid_channels, 3, padding=1, deformable_groups=stage1_groups)
        self.stage1_blocks = ResidualBlocksWithInputConv(3 * mid_channels, mid_channels, 3)

        # upsampling module
        self.reconstruction = ResidualBlocksWithInputConv(6 * mid_channels, mid_channels, num_reconstruction_blocks)
        ...
        self.aux_upsample1 = PixelShufflePack(mid_channels, mid_channels, 2, upsample_kernel=3)
        self.aux_upsample2 = PixelShufflePack(mid_channels, mid_channels, 2, upsample_kernel=3)
    ...
    def compute_flow(self, lrs):
        ...
        if self.is_mirror_extended:
            ...
            flows_forward = self.spynet(lrs_2, lrs_1).reshape([n, t - 1, 2, h, w])
    ...
    def stage1(self, feats, flows, flows_forward=None):
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i < t:
                ...
                cond_n1_ = flow_warp(feat_back, flow_n1_.transpose([0, 2, 3, 1]))
        ...
        for index in range(2):
            ...
            for i, idx in enumerate(frame_idx):
                ...
                if i > 0:
                    ...
                    feat_prop, offset, mask = self.deform_align[prop_name](feat_prop, cond, flow_n1)
                    pre_offset[prop_name][flow_idx[i]] = offset
                    pre_mask[prop_name][flow_idx[i]] = mask

                # concatenate and residual blocks
                feat = [feat_current] + [feats[k][idx] for k in feats if k not in ["spatial", prop_name]] + [feat_prop]
    ...
    def stage3(self, feats, flows, aux_feats=None, pre_offset=None, pre_mask=None):
    ...
    def auxiliary_stage(self, feats, lqs):
        ...
        for i in range(0, t):
            ...
            aux_feat = self.aux_block_down1(paddle.concat([hr, hr_high], axis=1))
            aux_feat = self.aux_block_down2(paddle.concat([aux_feat, hr_mid], axis=1))
            aux_feat = self.aux_fusion(paddle.concat([aux_feat, hr_low], axis=1))
    ...
    def upsample(self, lqs, feats, aux_feats=None):
        ...
        t = lqs.shape[1]
        for i in range(0, t):
            hr = [feats[k].pop(0) for k in feats if (k != "spatial" and k != "feat_stage1")]
        ...
        if self.auxiliary_loss:
            return paddle.stack(aux_feats["outs"], axis=1), paddle.stack(outputs, axis=1)
    ...
    def forward(self, lqs):
        ...
        feats, pre_offset, pre_mask = self.stage2(feats, (flows_backward, flows_forward))

        if self.auxiliary_loss:
            aux_feats = self.auxiliary_stage(feats, lqs)

        feats = self.stage3(feats, (flows_backward, flows_forward), aux_feats, pre_offset, pre_mask)
...
class ReAlignmentModule(nn.Layer):
    ...
    def forward(self, x, extra_feat, flow_1, feat_current, pre_stage_flow=None, pre_stage_mask=None):
...
class ModifiedSPyNet(nn.Layer):
    ...
    def __init__(self, act_cfg=dict(name="LeakyReLU"), num_blocks=6, use_tiny_block=False):
        super().__init__()
        self.num_blocks = num_blocks
        self.basic_module = nn.LayerList(
            [SPyNetBlock(act_cfg=act_cfg, use_tiny_block=use_tiny_block) for _ in range(num_blocks)]
        )
    ...
    def __init__(self, act_cfg=dict(name="LeakyReLU"), use_tiny_block=False):
        ...
        if use_tiny_block:
            self.basic_module = nn.Sequential(
                ConvLayer(in_channels=8, out_channels=16, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=32, out_channels=16, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=16, out_channels=8, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=8, out_channels=8, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=8, out_channels=2, kernel_size=3, stride=1, padding=1, act_cfg=None),
            )
        else:
            self.basic_module = nn.Sequential(
                ConvLayer(in_channels=8, out_channels=16, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ...
                ConvLayer(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=64, out_channels=32, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=32, out_channels=16, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ...
                ConvLayer(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, act_cfg=act_cfg),
                ConvLayer(in_channels=16, out_channels=2, kernel_size=3, stride=1, padding=1, act_cfg=None),
            )
