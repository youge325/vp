...
def flow_warp(x, flow, interpolation="bilinear", padding_mode="zeros", align_corners=True):
    ...
    if x_h != flow_h or x_w != flow_w:
        raise ValueError(f"The spatial sizes of input ({x.shape[-2:]}) and flow ({flow.shape[1:3]}) are not the same.")
    _, _, h, w = x.shape
    # create mesh grid
...
class SPyNet(nn.Layer):
    ...
    def compute_flow(self, ref, supp):
        ...
        flow = flow_up + self.basic_module0(
            paddle.concat(
                [ref[0], flow_warp(supp[0], flow_up.transpose([0, 2, 3, 1]), padding_mode="border"), flow_up], 1
            )
        )
        ...
        flow = flow_up + self.basic_module1(
            paddle.concat(
                [ref[1], flow_warp(supp[1], flow_up.transpose([0, 2, 3, 1]), padding_mode="border"), flow_up], 1
            )
        )
        ...
        flow = flow_up + self.basic_module2(
            paddle.concat(
                [ref[2], flow_warp(supp[2], flow_up.transpose([0, 2, 3, 1]), padding_mode="border"), flow_up], 1
            )
        )
        ...
        flow = flow_up + self.basic_module3(
            paddle.concat(
                [ref[3], flow_warp(supp[3], flow_up.transpose([0, 2, 3, 1]), padding_mode="border"), flow_up], 1
            )
        )
        ...
        flow = flow_up + self.basic_module4(
            paddle.concat(
                [ref[4], flow_warp(supp[4], flow_up.transpose([0, 2, 3, 1]), padding_mode="border"), flow_up], 1
            )
        )
        ...
        flow = flow_up + self.basic_module5(
            paddle.concat(
                [ref[5], flow_warp(supp[5], flow_up.transpose([0, 2, 3, 1]), padding_mode="border"), flow_up], 1
            )
        )
    ...
    def forward(self, ref, supp):
        ...
        flow = F.interpolate(flow_up, size=(h, w), mode="bilinear", align_corners=False)
...
class ResidualBlocksWithInputConv(nn.Layer):
    ...
    def __init__(self, in_channels, out_channels=64, num_blocks=30):
        ...
        # residual blocks
        self.ResidualBlocks = MakeMultiBlocks(ResidualBlockNoBN, num_blocks, nf=out_channels)
...
class SecondOrderDeformableAlignment(nn.Layer):
    ...
    def forward(self, x, extra_feat, flow_1, flow_2):
        ...
        offset_1 = offset_1 + flow_1.flip(1).tile([1, offset_1.shape[1] // 2, 1, 1])
        offset_2 = offset_2 + flow_2.flip(1).tile([1, offset_2.shape[1] // 2, 1, 1])
