    ...
    def __init__(self, in_nf=3, nf=64, HR_in=False):
        ...
        if self.HR_in:
            self.conv_first_1 = nn.Conv2D(
                in_channels=self.in_nf, out_channels=self.nf, kernel_size=3, stride=1, padding=1
            )
...
class TSAFusion(nn.Layer):
    ...
    def __init__(self, nf=64, nframes=5, center=2):
        ...
        self.Leaky_relu = nn.LeakyReLU(negative_slope=0.1)
        self.tAtt_2 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=3, stride=1, padding=1)
        self.tAtt_1 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=3, stride=1, padding=1)
        self.fea_fusion = nn.Conv2D(
            in_channels=self.nf * self.nframes, out_channels=self.nf, kernel_size=1, stride=1, padding=0
        )
        ...
        self.sAtt_2 = nn.Conv2D(in_channels=2 * self.nf, out_channels=self.nf, kernel_size=1, stride=1, padding=0)
        self.sAtt_3 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=3, stride=1, padding=1)
        ...
        self.sAtt_5 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=3, stride=1, padding=1)
        self.sAtt_add_1 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=1, stride=1, padding=0)
        self.sAtt_add_2 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=1, stride=1, padding=0)
        self.sAtt_L1 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=1, stride=1, padding=0)
        ...
        self.sAtt_L3 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=3, stride=1, padding=1)
        self.upsample = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False, align_mode=0)
...
class DCNPack(nn.Layer):
    ...
    def __init__(
        ...
        self.sigmoid = nn.Sigmoid()
        # init conv offset
        constant_init(self.conv_offset_mask, 0.0, 0.0)
...
class PCDAlign(nn.Layer):
    ...
    def __init__(self, nf=64, groups=8):
        ...
        self.PCD_Align_L3_offset_conv1 = nn.Conv2D(
            in_channels=nf * 2, out_channels=nf, kernel_size=3, stride=1, padding=1
        )
        self.PCD_Align_L3_offset_conv2 = nn.Conv2D(in_channels=nf, out_channels=nf, kernel_size=3, stride=1, padding=1)
        self.PCD_Align_L3_dcn = DCNPack(num_filters=nf, kernel_size=3, stride=1, padding=1, deformable_groups=groups)
        # L2
        ...
        self.PCD_Align_L2_offset_conv3 = nn.Conv2D(in_channels=nf, out_channels=nf, kernel_size=3, stride=1, padding=1)
        self.PCD_Align_L2_dcn = DCNPack(num_filters=nf, kernel_size=3, stride=1, padding=1, deformable_groups=groups)
        self.PCD_Align_L2_fea_conv = nn.Conv2D(in_channels=nf * 2, out_channels=nf, kernel_size=3, stride=1, padding=1)
        # L1
        ...
        self.PCD_Align_L1_offset_conv3 = nn.Conv2D(in_channels=nf, out_channels=nf, kernel_size=3, stride=1, padding=1)
        self.PCD_Align_L1_dcn = DCNPack(num_filters=nf, kernel_size=3, stride=1, padding=1, deformable_groups=groups)
        self.PCD_Align_L1_fea_conv = nn.Conv2D(in_channels=nf * 2, out_channels=nf, kernel_size=3, stride=1, padding=1)
        # cascade
        self.PCD_Align_cas_offset_conv1 = nn.Conv2D(
            in_channels=nf * 2, out_channels=nf, kernel_size=3, stride=1, padding=1
        )
        self.PCD_Align_cas_offset_conv2 = nn.Conv2D(in_channels=nf, out_channels=nf, kernel_size=3, stride=1, padding=1)
        self.PCD_Align_cascade_dcn = DCNPack(
            num_filters=nf, kernel_size=3, stride=1, padding=1, deformable_groups=groups
        )
...
class EDVRNet(nn.Layer):
    ...
    def __init__(
        ...
        self.Leaky_relu = nn.LeakyReLU(negative_slope=0.1)
        if self.predeblur:
            self.pre_deblur = PredeblurResNetPyramid(in_nf=self.in_nf, nf=self.nf, HR_in=self.HR_in)
        ...
        self.fea_L2_conv1 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=3, stride=2, padding=1)
        self.fea_L2_conv2 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=3, stride=1, padding=1)
        ...
        self.fea_L3_conv2 = nn.Conv2D(in_channels=self.nf, out_channels=self.nf, kernel_size=3, stride=1, padding=1)
        ...
        # TSA Fusion module
        if self.w_TSA:
            self.TSAModule = TSAFusion(nf=self.nf, nframes=self.nframes, center=self.center)
        else:
            self.TSAModule = nn.Conv2D(
                in_channels=self.nframes * self.nf, out_channels=self.nf, kernel_size=1, stride=1
            )

        # reconstruction module
        self.reconstructor = MakeMultiBlocks(ResidualBlockNoBN, self.back_RBs, self.nf)
        self.upconv1 = nn.Conv2D(in_channels=self.nf, out_channels=4 * self.nf, kernel_size=3, stride=1, padding=1)

        self.pixel_shuffle = nn.PixelShuffle(2)
        self.upconv2 = nn.Conv2D(in_channels=self.nf, out_channels=4 * 64, kernel_size=3, stride=1, padding=1)
        self.HRconv = nn.Conv2D(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.conv_last = nn.Conv2D(in_channels=64, out_channels=self.out_nf, kernel_size=3, stride=1, padding=1)
        if self.scale_factor == 4:
            self.upsample = nn.Upsample(
                scale_factor=self.scale_factor, mode="bilinear", align_corners=False, align_mode=0
            )
    ...
    def forward(self, x):
        ...
        aligned_fea = [
            self.PCDModule([L1_fea[:, i, :, :, :], L2_fea[:, i, :, :, :], L3_fea[:, i, :, :, :]], ref_fea_l)
            for i in range(N)
        ]
