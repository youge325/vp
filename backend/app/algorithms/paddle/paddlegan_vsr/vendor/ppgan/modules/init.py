...
def kaiming_init(layer, a=0, mode="fan_out", nonlinearity="relu", bias=0, distribution="normal"):
    assert distribution in ["uniform", "normal"]
    if distribution == "uniform":
        kaiming_uniform_(layer.weight, a=a, mode=mode, nonlinearity=nonlinearity)
    else:
...
def init_weights(net, init_type="normal", init_gain=0.02, distribution="normal"):
