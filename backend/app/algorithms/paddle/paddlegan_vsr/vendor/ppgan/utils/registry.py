...
def build_from_config(cfg, registry, default_args=None):
    ...
    if not isinstance(registry, Registry):
        raise TypeError(f"registry must be an ppgan.utils.Registry object, but got {type(registry)}")
    ...
    if isinstance(cls_name, str):
        ...
        raise TypeError(f"name must be a str or valid name, but got {type(cls_name)}")

    try:
        instance = obj_cls(**args)
    except Exception as e:
        stack_info = traceback.format_exc()
        print("Fail to initial class [{}] with error: {} and stack:\n{}".format(cls_name, e, str(stack_info)))
        raise e
    return instance
