...
特点：LeakyReLU 激活、ResConv 残差卷积、nn.Sequential Head(3→16→4)、
4 级 IFBlock、PixelShuffle 上采样、支持 ensemble。

v4.7 相比 v4.6 的变化：引入 Head 编码器（encode），IFNet.forward 接受 f0/f1。

参考 vs-rife 的 IFNet_HDv3_v4_7.py 实现。
"""
...
class IFBlock(nn.Module):
    ...
    ResConv + PixelShuffle 上采样，输出 (flow, mask) 2 元组。
    """
...
class IFNet(nn.Module):
    ...
    4 级 IFBlock，nn.Sequential Head 编码器（3→16→4），支持 ensemble。
    block0 输入 7+8=15 通道（img0+img1+timestep+f0+f1）。
    """
