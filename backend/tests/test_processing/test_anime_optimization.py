...
1. ``process_frame`` / ``process_frame_batch`` 不丢帧、不改帧 —— 占位
   实现以后真正落地算法时,这两条不变性要先撑住"identity baseline"。
...
4. ``validate() == True`` —— 用算法注册路径里的 sanity check。
...
def test_process_frame_batch_passes_through_unchanged() -> None:
    algorithm = AnimeOptimizationAlgorithm()
    frames = [
    ...
    result = algorithm.process_frame_batch(frames)

    # batch identity:同一个 list、同一批 ndarray。
    assert result is frames
    assert len(result) == 3
    for original, processed in zip(frames, result):
...
def test_validate_returns_true_for_placeholder() -> None:
    """算法注册链路上的 sanity check —— 占位实现应该 unconditionally OK,
    否则 Workflow 起步就被拒。"""

    assert AnimeOptimizationAlgorithm().validate() is True
