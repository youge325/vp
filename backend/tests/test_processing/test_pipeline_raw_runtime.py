...
class _FakeFFmpeg:
    def __init__(self) -> None:
        self.encoder_fps: list[float] = []

    def open_rawvideo_encoder(self, *, output_path: str, fps: float, progress_callback: Any = None, **kwargs: Any):
        del kwargs
        self.encoder_fps.append(fps)
...
def test_raw_pipeline_runtime_runs_encoder_thread_and_stage_worker(tmp_path: Path) -> None:
    ...
    runner_saw_runtime = False

    def fake_stage_worker_runner(**kwargs: Any) -> None:
        nonlocal runner_saw_runtime
        runner_saw_runtime = all(key in kwargs for key in ("encode_queue", "error_queue", "stop_event"))
