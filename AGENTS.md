...
近期提交使用简短祈使句主题，通常为英文句首大写，例如 `Fix benchmark model directory fallback`、`Add backend benchmark regression system`。保持每个 commit 聚焦于单一层面或单一问题。PR 应说明影响的 backend/frontend/Rust 范围，列出已执行的验证命令，关联相关 issue；涉及 UI 的改动需提供截图或录屏。单独说明生成文件变更，尤其是 Tauri schema 和 ts-rs 生成的 TypeScript 类型。
