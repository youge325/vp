...
FORBIDDEN_PATTERN_RULES = (
    ...
    *(
    ...
    _forbid(
        ...
        "frontend/src/services/preset/normalize.ts",
        r"\bpreferDefaults\b",
        "preset normalize exposes an unused default-preference strategy",
    ),
    _forbid(
        "preset-output-default-forwarder",
        "frontend/src/services/preset/defaults.ts",
        r"\bcreateDefaultOutputConfig\b",
        "single-consumer output default factory",
    ),
    _forbid(
        "media-run-reset-log-policy",
        "frontend/src/stores/mediaRunState.ts",
        r"\bpreserveLogs\b",
        "media runtime reset exposes an unused log-preservation policy",
    ),
    _forbid(
        "workflow-step-algorithm-override",
        "backend/app/planning/workflow_steps.py",
        r"def\s+(?:_resolve_algorithm_types|resolve_processing_steps)\s*\([^)]*\balgorithm\s*:",
        "workflow planning exposes an unused algorithm override",
    ),
    _forbid(
        "cli-process-resume-mode-fallback",
        "backend/app/cli/commands/process.py",
        r"getattr\s*\(\s*args\s*,\s*[\"']resume_mode[\"']",
        "process command tolerates an incomplete parser namespace",
    ),
    _forbid(
        "cli-config-stdin-fallback",
        "backend/app/cli/commands/_process_validation.py",
        r"getattr\s*\(\s*args\s*,\s*[\"']config_stdin[\"']",
        "config loader tolerates an incomplete parser namespace",
    ),
    ForbiddenReferenceRule(
        "obsolete-default-output-path-helper",
        roots=("backend/app",),
        patterns=(r"\bget_output_path\b",),
        message="output path helper with dead suffix and extension strategies",
        suffixes=(".py",),
    ),
    ForbiddenReferenceRule(
        ...
        roots=(
            ...
            "backend/app/processing/streaming/worker_process_events.py",
            "backend/app/processing/streaming/worker_processes.py",
            "backend/app/processing/streaming/worker_chain_runtime.py",
            "backend/app/processing/streaming/worker_pipeline.py",
            "backend/app/processing/streaming/encoder_worker.py",
        ...
        patterns=(
            r"queue\.Queue\[Any\]",
            r"\bstop_event\s*:\s*Any\b",
            r"\b(?:worker_stdin|final_stdout|pipe)\s*:\s*Any\b",
            r"\bprogress_callbacks\s*:\s*list\[Any",
            r"\b_writer\s*:\s*Any\b",
            r"def\s+write_frame\s*\([^)]*\bframe\s*:\s*Any\b",
        ),
        message="worker runtime uses a broad type at a concrete process boundary",
        suffixes=(".py",),
    ),
    _forbid(
    ...
    ForbiddenReferenceRule(
        ...
        roots=(
            "frontend/src-tauri/src/process_control/mod.rs",
            "frontend/src-tauri/src/tasks/controller.rs",
        ),
        patterns=(
            r"\bdefault_controller\b",
            r"DefaultProcessController::new\s*\(",
        ),
        message="single-consumer default process controller factory",
        suffixes=(".rs",),
    ),
    _forbid(
...
REQUIRED_PATTERN_RULES = (
    ...
    _require(
        ...
        "frontend/src-tauri/src/tasks/controller.rs",
        r"let\s+controller\s*=\s*DefaultProcessController::default\(\);[\s\S]{0,1000}if\s+let\s+Some\(timeout\)\s*=\s*parse_stall_timeout\(\)",
    ...
    _require(
        ...
        "frontend/src/composables/app/taskOrchestratorRuntime.ts",
        r"resetItemRunState:\s*\(id\)\s*=>\s*mediaRunState\.resetItemRunState\(id\)[\s\S]{0,180}resetItemsRunState:\s*\(ids\)\s*=>\s*mediaRunState\.resetItemsRunState\(ids\)",
        "task runtime does not fix single-item and final batch log reset policies",
    ),
    _require(
        "explicit-media-run-reset-semantics",
        "frontend/src/stores/mediaRunState.ts",
        r"function\s+resetItemRunState\(id:\s*string\):\s*void\s*\{\s*setIdleRunState\(id,\s*\[\]\)\s*\}[\s\S]{0,260}function\s+resetItemsRunState\(ids:\s*Set<string>\):\s*void[\s\S]{0,220}setIdleRunState\(id,\s*state\[id\]\?\.taskState\.logs\s*\?\?\s*\[\]\)",
        "media runtime store must make single-item and batch log reset semantics explicit",
    ),
    _require(
        "workflow-only-step-planning",
        "backend/app/planning/workflow_steps.py",
        r"def\s+_resolve_algorithm_types\(workflow_config:\s*dict\[str,\s*Any\]\)[\s\S]{0,700}return\s+\[\][\s\S]{0,500}def\s+resolve_processing_steps\(workflow_config:\s*dict\[str,\s*Any\]\)[\s\S]{0,160}algorithm_types\s*=\s*_resolve_algorithm_types\(workflow_config\)",
        "processing steps must be derived only from the workflow",
    ),
    _require(
        "direct-cli-parser-fields",
        "backend/app/cli/commands/process.py",
        r"resume_mode\s*=\s*args\.resume_mode",
        "process command must consume the parser-guaranteed resume mode directly",
    ),
    _require(
        "direct-config-stdin-field",
        "backend/app/cli/commands/_process_validation.py",
        r"if\s+args\.config_stdin:\s*return\s+_read_stdin_config_sections\(\)",
        "config loader must consume the parser-guaranteed stdin flag directly",
    ),
    _require(
        "default-output-path-contract",
        "backend/app/utils/file_utils.py",
        r"def\s+prepare_default_output_path\(\s*input_path:\s*str,\s*output_dir:\s*str,\s*container:\s*str,\s*\)\s*->\s*str:[\s\S]{0,360}os\.makedirs\(output_dir,\s*exist_ok=True\)[\s\S]{0,220}_processed\{extension\}",
        "default output path helper must require its three production inputs",
    ),
    _require(
        "typed-stage-progress-callback",
        "backend/app/processing/streaming/stage_worker_progress.py",
        r"class\s+StageProgressCallback\(Protocol\):[\s\S]{0,220}def\s+__call__\(self,\s*current:\s*int,\s*total:\s*int,\s*\*\*metadata:\s*Any\)\s*->\s*None",
        "stage progress callback protocol is missing",
    ),
    _require(
        "typed-encode-queue-contract",
        "backend/app/processing/streaming/queues.py",
        r"class\s+_EncodeEnd\b[\s\S]{0,180}type\s+EncodeQueueItem\s*=\s*EncodedFrame\s*\|\s*SegmentBoundary\s*\|\s*StreamEnd\s*\|\s*_EncodeEnd[\s\S]{0,120}type\s+EncodeQueue\s*=\s*queue\.Queue\[EncodeQueueItem\]",
        "encode queue item and terminal sentinel types are missing",
    ),
    _require(
        "typed-worker-process-io",
        "backend/app/processing/streaming/worker_process_io.py",
        r"class\s+DecodedFrameWriterConfig\b[\s\S]{0,500}\bffmpeg:\s*FFmpegWrapper[\s\S]{0,500}\bworker_stdin:\s*BinaryIO\s*\|\s*None[\s\S]{0,240}\bstop_event:\s*threading\.Event[\s\S]*def\s+drain_final_worker_output\b[\s\S]{0,900}\bencode_queue:\s*EncodeQueue",
        "worker process I/O must use concrete runtime types",
    ),
    _require(
        "typed-worker-event-handle",
        "backend/app/processing/streaming/worker_process_events.py",
        r"class\s+_WorkerEventHandle\(Protocol\):[\s\S]{0,260}\bprocess:\s*subprocess\.Popen\[bytes\][\s\S]{0,180}\bplan:\s*StageWorkerPlan[\s\S]{0,900}progress_callbacks:\s*Sequence\[StageProgressCallback\s*\|\s*None\]",
        "worker stderr events must consume a typed process handle and callbacks",
    ),
    _require(
        "typed-encoder-segment-writer",
        "backend/app/processing/streaming/encoder_segment_writer.py",
        r"self\._writer:\s*RawVideoWriter\s*\|\s*None[\s\S]{0,300}def\s+write_frame\(self,\s*frame:\s*np\.ndarray\)\s*->\s*None",
        "encoder segment writer must use concrete writer and frame types",
    ),
    _require(
