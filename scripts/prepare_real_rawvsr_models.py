"""Download, verify, and deterministically convert official Real-RawVSR RGB weights."""
...
def _source_checkpoint(
    ...
    candidates = (
        ...
        source_dir / f"{source_stem}-x{scale_factor}-best.pth",
        source_dir / f"{source_stem}-x{scale_factor}.pth",
    )
    for candidate in candidates:
        if candidate.is_file():
            return candidate
    raise RuntimeError(
        f"No {algorithm_id} x{scale_factor} source checkpoint found under {source_dir}; expected one of "
        + ", ".join(str(candidate) for candidate in candidates)
    )


def _serialize_state_dict(checkpoint_path: Path) -> tuple[bytes, int]:
    import torch
    ...
    return first, sum(tensor.numel() for tensor in tensors.values())
...
def prepare_models(*, acceptance: str, source_dir: Path | None = None) -> None:
    ...
    for field in ("noticeRelativePath", "licenseRelativePath", "thirdPartyNoticeRelativePath"):
        license_path = REPO_ROOT / manifest["license"][field]
        ...
        for family in manifest["families"]:
            ...
            for variant in family["variants"]:
                ...
                _verify(
                    ...
                    label=f"{label} official checkpoint",
                )
                converted, parameter_count = _serialize_state_dict(source_path)
                if parameter_count != int(variant["parameterCount"]):
                    raise RuntimeError(
                        f"{label} parameter count mismatch: expected {variant['parameterCount']}, got {parameter_count}"
                    )
                if len(converted) != int(variant["inferenceBytes"]):
                    raise RuntimeError(
                        f"{label} SafeTensors size mismatch: expected {variant['inferenceBytes']}, got {len(converted)}"
                    )
                converted_sha256 = hashlib.sha256(converted).hexdigest()
                if converted_sha256 != variant["inferenceSha256"]:
                    raise RuntimeError(
                        f"{label} SafeTensors SHA-256 mismatch: "
